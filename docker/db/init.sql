...
BEGIN
    ...
    ELSE
        ...
        SET @code = get_code('member', @member_id);
...
CREATE PROCEDURE register_client(_email VARCHAR(512), _client_id VARCHAR(64), _name VARCHAR(256), _description VARCHAR(512), _redirect_uri VARCHAR(512))
BEGIN
    SET @member_id = (SELECT `id` FROM `member` WHERE `email` = _email);

    IF @member_id IS NULL THEN
        SELECT 'member_not_found' as `error`;
    ELSEIF (SELECT COUNT(*) FROM `client` WHERE `id` = _client_id LIMIT 1) = 1 THEN
        SELECT 'client_exists' as `error`;
    ELSE
        INSERT INTO `client` (`member_id`, `id`, `name`, `description`, `redirect_uri`, `date_created`)
        VALUES (@member_id, _client_id, _name, _description, _redirect_uri, NOW());
        COMMIT;

        SET @code = get_code('client', _client_id);
        SELECT @code as `code`, _client_id as `client_id`;
    END IF;
END //
...
BEGIN
    ...
    ELSE
        SET @code = get_code('auth', @member_id);
        SELECT @code as `code`, @member_id as `member_id`;
    END IF;
END //

DROP PROCEDURE IF EXISTS insert_code //
CREATE PROCEDURE insert_code(_service VARCHAR(10), _service_id INT, _code VARCHAR(32))
BEGIN
    INSERT INTO `code` (`service`, `service_id`, `code`, `date_created`)
    VALUES (_service, _service_id, _code, NOW());
    COMMIT;
END //

DROP FUNCTION IF EXISTS get_code //
CREATE FUNCTION get_code(_service VARCHAR(10), _service_id INT)
...
BEGIN
    ...
    CALL insert_code(_service, _service_id, @code);
